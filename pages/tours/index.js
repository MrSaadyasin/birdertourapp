...
import { allTours } from "../../constants/endpoint.constants";
...
function Tours(props) {
  ...
  const searchByLocation = async () => {
    setLoading(true);
    try {
      const searchByAddress = await props?.dispatch(SearchByLocation(router.query));
  ...
  const searchVendor = async () => {
    setLoading(true);
    try {
      const searchByVendor = await props?.dispatch(SearchByVendor(router.query));
  ...
  useEffect(() => {
    if (router.query && router.query.type === "location") {
      setAddress(router.query.address);
      setValue("By Location");
      searchByLocation();
    } else if (router.query && router.query.type === 'vendor') {
      setValue("By Vendor");
      setVendorText(router.query.name)
      searchVendor()
    }
  }, [router.query])
  ...
  const searchByVendor = async (e) => {
    ...
    try {
      ...
      const searchByVendor = await props?.dispatch(SearchByVendor(payload));
      setTour(searchByVendor?.payload?.data?.tours);
    } catch (e) {
      console.log(e);
      setTour([]);
  ...
  const handleOnTourLocation = async (e) => {
    ...
    if (e?.formatted_address) {
      ...
      try {
        const searchByAddress = await props?.dispatch(SearchByLocation(payload));
        setTour(searchByAddress?.payload?.data?.tours);
      } catch (e) {
        console.log(e);
        setTour([]);
  ...
  return (
    <>
      <PageLayout color="#1b847a" footer={true} HomeSearchBar={false}>
        <ToastContainer />
          ...
          <div className="padding120 px-0 pb-0">
            ...
            <div className="d-flex position-relative">
                ...
                {isMobileDevice ? (
                  ...
                  <>
                    <input
                      ...
                      readOnly
                      placeholder={`${value === "By Vendor" ? t("ByName") : t("ByLocation")}`}
                    />
                    <div className="caretIcon">
                      <svg
                        width="20"
              ...
              {showDD && (
                <>
                  <div
                    className="dropdownMenu shadow"
                    style={{ right: value == "By Vendor" && "12%" }}
                  >
                    <div
                      style={{
...
export default connect(mapStoreProps)(Tours);
