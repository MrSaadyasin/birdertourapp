...
const DiscoverBird = ({ tours, dispatch, setSearchTours, setSearch, discoverBirdRef, HomeScreen }) => {
    ...
    if (HomeScreen) {
      ...
      if (e?.formatted_address) {
        ...
        try {
          ...
          setSearchTours(searchByAddress?.payload?.data?.tours)
        } catch (e) {
          console.log(e);
          setSearchTours([]);
          setTour([]);
        } finally {
          if (discoverBirdRef.current) {
            discoverBirdRef.current.scrollIntoView({ behavior: "smooth" });
          }
        }
      } else {
      ...
      // router.push('/tours?location=Bangladesh')
      router.push({
        pathname: '/tours',
        query: {
          type: 'location', latitude: e?.geometry?.location?.lat(),
          longitude: e?.geometry?.location?.lng(), address: countryFullNamesToAbbreviations[e.formatted_address] || e.formatted_address
        },
      })
  ...
  const searchByVendor = async (e) => {
    try {
      ...
      setSearchTours(searchByVendor?.payload?.data?.tours);
    } catch (e) {
      console.log(e);
      setSearchTours([]);
    } finally {
  ...
  return (
    <>
      <section className={`${style.sectionDiscoverSearch}`}>
        <div className="pb-0">
          <div className="d-flex position-relative">
            {value === "By Location" ? (
              ...
              >
                ...
                <ReactGoogleAutocomplete
                  ...
                  onChange={(e) => {
                    if (!e.target.value && HomeScreen) {
                      if (tours?.tours?.length > 0) {
                        setTour(tours?.tours);
                        setSearchTours(tours?.tours);
                      } else {
                        setTour([]);
                        setSearchTours([]);
                      }
                  ...
                  className="form-control customInputSearch border-0 whitePlaceHolder"
                  options={{
                    types: [`(regions)`],
                  }}
                />
              ...
              >
                <input
                  className="form-control   border-0 customInputSearch whitePlaceHolder "
              ...
              {isMobileDevice ? (
                ...
                <>
                  <input
                    className={
                      value == "By Name"
                        ? "form-control customInputSelect border-0 forVendor whitePlaceHolder"
                        : "form-control customInputSelect border-0 whitePlaceHolder"
                    }
                    readOnly
            ...
            {showDD && (
              <>
                <div className="dropdownMenu shadow" style={{ right: value == "By Name" && "12%", width: "30%" }}
                >
                  <div
                    style={{
        ...
        {/* {tour && tour?.length > 0 ? (
          ...
          <div className="d-flex align-items-center justify-content-center my-lg-5 py-5">
            <div className="text-center">
              ...
              />
              <h2>
                <strong>No tours are available!</strong>
              </h2>
              <p>There are no tours available at the moment!</p>
            </div>
          </div>
        )} */}
      </section>
