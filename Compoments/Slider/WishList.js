...
const WishListCard = ({ tour, handleLike, margin }) => {
  const array = JSON.parse(tour?.tour?.location)?.address?.split(",");
      ...
      >
        <div className="position-relative" style={{height: 250}}>
          {tour?.tour?.video ? (
            ...
            <Tooltip
              title={
                tour?.tour?.caption_images[0]?.caption
                  ? tour?.tour?.caption_images[0]?.caption
                  : "N/A"
              }
            >
              <Image
                src={tour?.tour?.caption_images[0]?.image}
                width={250}
                height={310}
                className={style.bgImage}
                style={{
          ...
          <div
            ...
            onClick={(event) => handleLike(event, tour?.tour?._id)}
          >
            <svg
              width="20"
              height="18"
              ...
              <div className="d-flex align-items-center pb-1">
                ...
                <div className={style.addrerss}>
                  ...
                  {tour?.tour?.rating
                    ...
                    : "(0)"} ,{" "}
                  {
                    JSON.parse(tour?.tour?.location)?.address?.split(",")[
                      array.length - 1
                    ]
                  }
                </div>
              </div>
              <p className={style.guider}>
                Trip Guide: {tour?.tour?.vendor?.name}
              </p>
